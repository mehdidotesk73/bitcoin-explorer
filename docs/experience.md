...
### 2026-06-02 — Indicator setup (metric registry + state persistence)
Merge of the Indicator setup work (building on the metric-framework branch).
Changes vs the Run/scale metric-framework version:

- **Added:** `lib/metricRegistry.ts` — metric registry spec system. `MetricSpec`
  interface (id, label, kind = overlay | curve, params, sharedParamGroup).
  `ParamSpec` for flexible types (number with slider bounds, period with
  unit dropdown). `MetricState` interface (enabled flags, param values, period
  units). Persistence functions: `defaultMetricState()`, `loadMetricState()`,
  `saveMetricState()`. URL encoding: `encodeMetricState()`, `decodeMetricState()`
  for shareability.
- **Refactored:** `PriceExplorer.vue` to use the metric registry. Replaced
  individual toggle refs (showMa, showBb, etc.) with a single `metricState` ref.
  Replaced hard-coded metric-card UI with data-driven v-for rendering over
  `METRIC_SPECS`. Dynamic param-control rendering: period controls (number +
  unit dropdown), sliders (for scale/sensitivity/continuous params), number
  inputs. Auto-persist to localStorage on change (watch with deep: true).
  Load from URL query params first (for shareability), then localStorage, then
  defaults. Added clipboard-copy button (📋) to generate shareable URL with
  encoded state.
- **Updated:** all derived computed properties to read from metricState
  (maPeriod, maUnit, bbPeriod, bbUnit, bbK, ratioMaDays, runScaleDays,
  sustThresh).
- **Impact:** Adding a new metric now requires one entry to the registry; the
  UI renders itself. State is automatically persisted and shareable via URL.
  Foundation for Hodl Explorer's metric-picker (which reads from the registry).
...
- **Docs:** added `CLAUDE.md` and `experience.md`; updated `docs/TODO.md` to
  retire the old DCA Explorer plan and queue the Indicator setup + Hodl Explorer
  phases.
